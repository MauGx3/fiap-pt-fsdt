...
router.post('/', authenticateUser, createPost);                                    // Create post
router.put('/:id', authenticateUser, updatePost);                                  // Update post (author or admin only)
router.delete('/:id', authenticateUser, requireRole('admin', 'author'), deletePost); // Delete post (admin or author only)
